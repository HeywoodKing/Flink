...
var fk = {
	...
	export: function(opts){
		try{
			var f = this;
			f.setOptions.call(f, opts);

			var downloadFileName = f.options.fileName;
			var ext = f.options.fileName.extension();

			var re = {};
			switch(f.options.type.toUpperCase()){
				case 'EXCEL':
					// re = f.excel(f.options);
					re = f.excelBlob(f.options);
					break;
				case 'WORD':
					re = f.word(f.options);
					break;
			...
			if(re.uri != ""){
				...
				// window.location.href = re.uri + f.base64(re.template);

				var link = document.createElement("a");
				link.style = "display: none;"
				link.href = re.uri;// + f.base64(re.template);
				// 下载的文件名
